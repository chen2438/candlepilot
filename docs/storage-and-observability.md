...
- 独立分析历史表现不新增或回写收益表；读取时联结 `market_analyses` 与最新
  ...
  止损金额只改变本次响应的仓位假设，不修改历史分析、结果或数据库。结果刷新会在既有 outcome JSON
  中保存分析完成后的首个完整 1m K 线开盘价；全部方向计划统计在读取时用它为未入场终态模拟入场，
  汇总明确的已入场/未入场盈亏并排除 `ambiguous`，无需数据库迁移。旧 outcome 缺该字段时不猜测，
  仍计方向胜负但需重新刷新结果后才计价。
...
- AI 分析辅助正式周期同时写入 `market_analyses` 与正式推理/风控审计，不新增重复策略表。分析
  ...
  每个辅助结果还以 `assisted_execution_ready=true/false` 标明方向计划是否具备完整开仓提示；neutral
  记为空。缺提示产生的安全 `HOLD` 仍保留原始模型分析和输出，便于区分“模型选择观望”与“执行字段不全”。
