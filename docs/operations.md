...
服务进程启动时从安装仓库执行固定的本地只读 `git rev-parse --short=7 HEAD`，不访问远端，所得
提交号会在每次正式运行创建时固化为该运行的软件版本。Git 不可用、目录不是仓库或返回值异常时
不阻止服务和交易运行，只把新运行标记为“版本未记录”；历史运行永远不按当前 `HEAD` 回填。
...
修改任何文件前被拒绝，避免遗漏数据库备份或锁错文件。更新模式不重写 Nginx、TLS、主应用 systemd unit 或操作系统包，
但会安装固定的 `candlepilot.service.d/logging.conf` 日志命名空间 drop-in，并
...
drop-in、在没有活动交易/模型任务时短暂重启主服务，并只对 `--namespace=candlepilot` 执行
rotate/vacuum。它不接受路径、unit 或保留期，也不会清理默认 journal。旧部署首次点击会完成隔离；
升级前留在默认 journal 的旧记录无法按 unit 单独删除，需由管理员按整机日志策略处理。
...
登录有效后卡片会自动读取 Codex CLI 实际报告的剩余额度，也可手动刷新；Team 只报告月额度时页面
只显示月额度。额度读取依赖 app-server 实验接口，旧版或不兼容版本会显示暂不可用，可先更新 CLI；
该降级不阻止既有模型调用，也不需要重新登录或重启 CandlePilot。
...
loopback 后端端口。
服务日志用 `journalctl --namespace=candlepilot -u candlepilot.service -f` 查看。若 UFW 已经启用，脚本只增加所选 TCP 端口规则，
