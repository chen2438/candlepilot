...
- **本地规则（`local-rule` / `trend-v2`）**：不调用外部 LLM，不新增行情采集，直接读取正式决策
  ...
  该策略忽略仅实盘存在的盘口、主动成交、基差、持仓量和实验性多空比/OI 变化字段，保证普通回测
  与正式决策语义一致。实验性持仓字段也从标准外部 Provider payload 剔除，只在正式快照中冻结，
  待确定性影子策略与正式回放取得证据后再决定是否进入执行决策。
...
- 独立分析 Prompt 版本 `market-analysis-v3` 只允许读取所附冻结数据包，要求先看价格结构，再用 EMA/MACD/VWAP/量能、资金流与期权结构
