...
- **本地规则（`local-rule` / `trend-v2`）**：不调用外部 LLM，不新增行情采集，直接读取正式决策
  ...
  该策略忽略仅实盘存在的盘口、主动成交、基差、持仓量和多空比/OI 变化字段，保证普通回测
  与正式决策语义一致。外部 LLM 则会收到正式快照中实际存在的衍生品统计；程序把它们从普通
  技术特征移入独立 `derivatives_context`，同时给出来源、5m 已收盘口径、完整性、缺失清单和
  解释限制，避免同一读数重复出现或由缺失值触发暗含的零值判断。
...
- **Custom API（可多个）**：全部端点统一由 `CANDLEPILOT_CUSTOM_LLM_PROVIDERS_JSON` 定义
  ...
  （各自的地址、密钥、模型与协议）。注册名 `openai-compatible:<id>`，选作运行 Provider 时写
...
- 统一交易 Prompt 明确说明 `PortfolioState.stop_loss_cooldown_until` 是最近 90 分钟发生手续费后
  ...
  英文机器值；`rationale` 使用简体中文。`portfolio.position_entry_context` 附带所有当前持仓
  对应的已成交开仓/加仓理由，以及本次正式运行内全部已成交开仓/加仓和后续退出；每条退出保留
  `take_profit`、`stop_loss`、`manual_close`、模型平/减仓、紧急回补或其他平仓的审计类别、
  分配数量与可用的已实现盈亏。Prompt 明确禁止补猜缺失理由或把手动/模型退出解释成止盈止损。
  当前 Prompt 版本为 `trade-intent-v23`。
...
- **行情分析结构化调用**：Codex、Claude Code 和 Custom API 还实现供独立研究与分析辅助 SHADOW 使用的
  ...
  neutral 必须给包含锚价的数字区间且不得有入场；若区间仅在锚点一侧且锚点到近端边界的距离不超过
  原区间宽度，程序可把近端边界扩展至锚点，因为该字段不产生交易；偏离更远仍拒绝。
  方向性计划必须显式给 entry/stop/T1/T2 并满足方向几何；Prompt 仍要求 T1 原始盈亏比至少 1，
  但低于该建议值不再作为结构化协议错误拖垮整批。正式辅助模式把原值交给逐标的 1.15R 硬风控，
  独立研究则保存并明确展示弱计划，不替模型改价。提供给 Codex/OpenAI 严格结构化输出的整棵 Schema 会递归把
  每个对象的全部属性列入 `required` 并禁止额外字段；即使 `missing_data_impact` 为空也必须显式
  返回空数组，避免模型调用在推理前被协议拒绝。缺字段不会按固定百分比自动补齐。
- 独立分析 Prompt 版本 `market-analysis-v4` 只允许读取所附冻结数据包，要求先看价格结构，再用 EMA/MACD/VWAP/量能、资金流与期权结构
  确认；止损必须越过命名结构，T1 约减半、结构允许时余仓止损向保本移动，并以约 6 根 15m 锚定
  K 线无进展作为重新评估条件。Prompt 与输出 Schema 的字段描述同时要求所有面向用户的字符串值使用
  简体中文，同时保留英文 JSON 键、枚举、标的、周期、数字及市场标准缩写；不增加翻译调用，也不
  改写历史记录。期权只能确认或削弱情景：模型必须区分 `direct` 本标的与 BTC/ETH `market_benchmark`，
  不得把 OI 解释成带方向仓位、把大 OI 行权价当成已证实支撑阻力，或把 Put/Call 与 IV 单独当成方向。
  数据包中的 Binance 5m 持仓统计同样带完整性与缺失清单；模型只能结合价格结构综合解释价格、
  OI 变化、多空比和 taker 主动成交，不能套用固定阈值。缺失统计必须作为未知影响披露。
  以上全部是研究计划，不会调用硬风控、Broker 或自动执行。
- 独立分析的批量入口是多个单标的分析组成的受控串行队列，不把不同标的塞入一个超大 Prompt，也不
  复用正式交易的批量 `TradeIntent` Schema。这样每项仍有独立数据时点、上一份同标的分析、Token、
  原始输出和失败状态；同一个外部 Provider 在整批期间持续占用唯一模型任务槽。每 15 分钟自动研究
  复用同一串行队列与并发锁，只在发起前增加“最近方向计划必须已经终结”的确定性筛选，不改变
  Prompt、Schema 或每标的一次物理 Provider 调用的审计口径。
- **AI 分析辅助正式决策（`analysis-assisted-decision-v5`）**：仅支持外部 Provider，并只开放
  ...
  让它拖垮同批其他标的；neutral 意外携带的提示一律忽略。辅助 Prompt 与独立分析使用相同的期权角色、
  缺失数据及非方向性解释边界。已有反向持仓仍只生成风险降低的平仓意图，
  不依赖开仓提示。启动试跑走
...
- **单一运行 Provider**：前端使用互斥选择；为兼容现有部署保留变量名
  ...
  明确拒绝，不会跳过配置或静默改选其他 Provider。选中的卡片已经完整表达唯一运行来源，前端不再
  额外显示“本次运行 Provider”路由列表或“实际承载”摘要；状态接口仍保留单元素数组结构，避免
  无必要地破坏客户端。
- **周期批量分析**：正式运行按唯一选定的 cadence 收集全部候选与已有持仓的完整行情快照，
  再读取一份共同账户状态；外部 Provider 用**一次物理调用**接收 `markets` 数组与单份
  `portfolio`。调用前引擎从已持久化推理、执行和用户流成交构造上述开平仓历史，因此 Provider
  重试时也会随刷新后的组合重新生成，而不是沿用过时状态；模型按输入顺序返回等长的 `intents`
  数组。数量、顺序、标的或周期任一不匹配都视为
...
- **故障冷却、同批次重试与恢复**：所选 Provider 调用失败后记录 60 秒重试冷却，但不等待下一个
  K 线周期，而是在同一决策内按 5 秒、15 秒退避再试两轮；每一轮都只调用同一个 Provider，不会
  转移给其他 Provider，也不让 60 秒的跨决策冷却吞掉本次
  ...
  不下单。任一轮成功即结束重试、清零连续失败计数并恢复承载；三轮仍失败才产生最终 `HOLD`。
  Provider 调用用全局锁串行，达到失败阈值后排队中的其他标的不会继续烧调用；所选 Provider 在
  引擎运行时锁定，防止并发分析期间改变选择。
- **失败审计**：每个实际发起但失败的 Provider 调用均单独写入推理审计，记录当前是第几轮、是否
  继续重试、错误、原始输出和可获得的 Token/耗时；最终成功结果再独立进入硬风控。三轮均失败时
  最后一次失败生成 `HOLD`，不会下单。兼容 API 中该对象仍沿用历史字段名 `failover`，但只表示
  所选 Provider 的失败调用，不代表发生 Provider 切换。
...
- **正式运行启动试跑**：持续调度前，用户可以先通过独立的 `POST /api/engine/probe` 试跑，再通过
  ...
  实时行情快照、当前测试网组合及唯一所选 Provider，执行 1 次真实的**全标的批量意图**。试跑
  不经过风控、不下单、不写入正式决策审计，但外部
  调用会产生真实 Token/计费。调用失败或超时即拒绝启动。系统使用
  “全部行情 + 一份账户 + 一次批量意图生成”的真实墙钟耗时；超过所选周期即返回
  ...
  试跑成功只解锁一次持续启动，不会自动交易；周期、每周期候选标的数、运行上限、Provider 选择、模型配置、
...
- **运行一次**：`POST /api/engine/run-once` 不等待下一个 K 线边界，立即使用唯一所选周期执行一轮
  完整流程：使用当前候选池（为空时先刷新）并重新读取账户、批量调用唯一所选 Provider、逐标的硬风控，
  并对放行意图正常提交测试网订单
