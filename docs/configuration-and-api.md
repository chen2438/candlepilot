...
| `CANDLEPILOT_PROVIDER_CHAIN` | 启动时唯一使用的 Provider，例如 `local`、`local-structure`、`local-flow`、`local-structure-flow`、`codex`、`claude-code` 或 `custom:main`；三个实验本地值在正式运行中强制只影子审计；保留该变量名用于兼容现有部署，但值必须恰好一个，不得用逗号配置主备，Custom API ID 必须存在；状态/API 返回注册名，不允许写回该变量 |
...
Provider；当前三个本地实验 Provider 返回 `true`，标准本地规则和外部 Provider 返回 `false`。
