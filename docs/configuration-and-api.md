...
| `CANDLEPILOT_PROVIDER_CHAIN` | 启动时唯一使用的 Provider，例如 `local`、`codex`、`claude-code` 或 `custom:main`；`local` 只表示确定性基准，不再提供结构/资金流固定阈值变体；保留该变量名用于兼容现有部署，但值必须恰好一个，不得用逗号配置主备，Custom API ID 必须存在；状态/API 返回注册名，不允许写回该变量 |
...
`POST /api/decision-events/{inference_id}/counterfactual-outcome`、
...
USDT 盈亏、平均回报或 R、胜率和样本计数；这两组顶层金额口径只统计已入场并明确结算的 stop、T2、
T1 后保本结果。响应另以 `all_plans` 返回全部方向计划的明确结果数、已入场/未入场数、胜负数、命中率、
已计价数，以及两套金额假设的累计盈亏与 R：
未入场 T1 与入场后到达 T1 计为命中，未入场 stop 与入场后 stop 计为未命中；观望、进行中和
`ambiguous` 均排除。未入场终态使用结果中的 `completion_entry_price`，即分析完成后的首个完整 1m K 线
开盘价，T1 前置按全仓 T1、stop 前置按全仓 stop 结算；旧结果缺该字段时仍保留方向胜负但不进入
`priced_plans`。`costs_included=false` 明示金额测算未计费用、滑点和资金费。
...
刷新每个候选最近成功方向分析的计划结果，等待入场、active、T1 部分止盈或刷新失败的标的不会创建
新记录；顺序不确定视为终态，与其他已经终结及 neutral 标的一样进入逐标的串行队列。停止开关不取消已创建的
...
`GET /api/providers` 的 `capabilities.live_shadow_only` 标识正式运行中只审计、不调用 Broker 的
Provider；当前内置 Provider 都返回 `false`，该能力字段仅供协议兼容和显式注入实现。
对 Codex 还返回当前 `auth_source`、已安装的 `auth_source_options` 和通过认证后
