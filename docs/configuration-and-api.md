...
USDT 盈亏、平均回报或 R、胜率和样本计数；这些金额口径只统计已入场并明确结算的 stop、T2、T1 后
保本结果。响应另以 `all_plans` 返回全部方向计划的明确结果数、已入场/未入场数、胜负数及命中率：
未入场 T1 与入场后到达 T1 计为命中，未入场 stop 与入场后 stop 计为未命中；观望、进行中和
`ambiguous` 均排除。`costs_included=false` 明示金额测算未计费用、滑点和资金费。
