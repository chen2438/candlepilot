...
- **配置连通性测试**：每个 Provider 可经前端「测试」按钮或 `POST /api/providers/test`
  ...
  引擎运行时锁定（返回 409）。
- **正式运行启动试跑**：用户先通过独立的 `POST /api/engine/probe` 试跑，再通过
  `POST /api/engine/start` 启动，或通过 `POST /api/engine/run-once` 立即运行一个交易周期。试跑使用
  ...
  **全标的批量意图**。试跑不经过风控、不下单、不写入正式决策审计，但外部
  ...
  试跑成功只解锁一次“运行一次”或持续启动，不会自动交易；周期、每周期候选标的数、运行上限、Provider 路由、模型配置、
...
| 总览 | 01 模型接入 | 宽屏左栏。每张 Provider 卡片内集成接入状态；外部模型可配置模型与推理强度，Codex 额外显示并可选择 ChatGPT App / Codex CLI 接入来源且显示当前登录邮箱，本地规则显示固定版本与零 Token/成本；下方提供有序主备路由及当前承载/冷却状态。浏览器宽度不超过 1200px 时，总览双栏提前折叠为单栏，避免缩放后的 Provider 配置控件越界覆盖右栏 |
| 总览 | 02 硬风控边界 | 宽屏右栏顶部，只读集中展示当前候选标的数、10 倍最大杠杆、5% 滚动 24 小时亏损熔断、1% 单笔风险、4% 组合止损风险、原始盈亏比必须大于 1.3:1、保证金与持仓模式等边界 |
| 总览 | 03 动态候选池 | 宽屏右栏紧接硬风控，填充模型列表右侧空间；展示全市场扫描结果，可手动刷新；默认只显示评分前 5 个，可展开查看全部 |
