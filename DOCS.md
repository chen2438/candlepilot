...
- **正式运行边界持久化**：每次通过启动检查后都会创建唯一 `live_run_id`，本次 Provider 成功、
  ...
  `running` 记录会在下一次服务启动时明确标为 `interrupted`，不冒充仍在运行。升级前产生的推理
  保持 `live_run_id=NULL`，前端归入「历史记录 · 未归属运行」，不按时间间隔猜测归属。
...
| 总览 | 04 决策与风控 | 按持久化正式运行分组并显示运行编号、起止时间、状态、配置摘要与停止原因；组内将 LLM 意图、硬风控与执行尝试合并为一条审计事件，可按放行、下单成功、下单失败、否决、HOLD、仅推理筛选，并展开失败阶段、交易所错误、紧急回补和损失估算；升级前记录明确归入「未归属运行」 |
