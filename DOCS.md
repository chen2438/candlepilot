...
- 开仓与 `ADD` 的**有效盈亏比硬下限为 1.3:1**，同时提示 Provider 以至少 1.5:1 为目标。
  该比例不采用模型自报值，而是在止损/止盈按交易所 tick 对齐后，以最新买卖价、双边 0.1%
  保守滑点和双边 0.05% 手续费重新计算；市价入场使用最新卖一（多）/买一（空），可立即成交的
  限价入场不会假设突破委托价成交。`ADD` 使用新增数量定量后的合并持仓加权均价与替换后的整仓
  止损/止盈计算，不能用新增部分单独伪造高盈亏比。`REDUCE/CLOSE` 不受盈亏比、日亏熔断或
...
- **AI 原始盈亏比可见**：决策细目按 Provider 返回的入场价、止损和止盈直接计算并显示原始
  ...
  不包含交易所 tick 对齐、最新盘口、手续费或滑点，不能替代硬风控按执行条件重算的有效盈亏比，
  因此原始值高于 1.3:1 的意图仍可能被有效盈亏比护栏否决。
...
| 总览 | 02 硬风控边界 | 宽屏右栏顶部，只读展示 1% 单笔风险、4% 组合止损风险、1.3:1 有效盈亏比、保证金与持仓模式等不可修改参数 |
