...
- **周期批量分析**：正式运行的每个 cadence 先收集本周期全部候选与已有持仓的完整行情快照，
  ...
  跨 cadence 对重叠标的使用规范顺序的联合锁，禁止另一个周期在整批分析期间对同一标的生成
  冲突决策。
...
- **正式运行启动试跑**：用户先通过独立的 `POST /api/engine/probe` 试跑，再通过
  `POST /api/engine/start` 启动。试跑使用候选前 N 与已有持仓的去重并集、所选最短周期的完整
  ...
  **全标的批量意图**。试跑不经过风控、不下单、不写入正式决策审计，但外部
  ...
  “全部行情 + 一份账户 + 一次批量意图生成”真实墙钟耗时；超过任一所选周期即返回
  422，要求减少标的或扩大周期。多周期还计算
  `批量最慢耗时 × Σ(1 / 周期秒数)` 的单并发总负载，超过 100% 同样拒绝，避免各周期
  单独看似可容纳、叠加后仍排队。响应与 `/api/status.startup_probe` 保存本次耗时、最慢值、
  ...
  试跑成功只解锁一次启动，不会自动启动；周期、每周期候选标的数、运行上限、Provider 路由、模型配置、
  正式决策硬超时或候选池变化后立即失效，必须按新参数重新试跑。成功启动会消费本次试跑，停止后
  再次启动同样需要重新试跑。后端也强制检查该状态，不能绕过前端直接启动。
...
- 5m/15m/30m/1h/4h 按各自 UTC K 线边界对齐调度；每周期把候选池前 N 个标的与全部已有持仓去重后组成一次批量分析，确保掉出候选池的
...
| 标签页 | 面板 | 内容 |
|---|---|---|
| 总览 | 引擎控制（hero）| 系统状态、分析周期、每周期候选标的数、正式决策硬超时、一次真实批量试跑的候选/额外持仓/实际分析数量、容量与调用详情、当前周期/标的/阶段，以及相互独立的试跑、启动、停止、紧急熔断按钮；宽屏为状态与控制两栏，四个控制按钮并排；下方实时显示本次或上次运行的 Token、等效成本、调用数与时长 |
...
非零持仓；测试网将过去 24 小时交易流水与当前未实现盈亏合并为 `pnl_24h`，不再按 UTC 日期切换
