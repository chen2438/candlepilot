...
- **故障冷却、同决策重试与恢复**：一轮分析按顺序尝试未冷却节点；调用失败的节点立即冷却
  ...
  恢复机会。每个新重试轮在退避结束后重新获取该标的的完整行情快照和当前测试网组合，后续
  Provider 调用与最终时效检查都使用这份新输入，不能让前一轮的模型耗时消耗下一轮的快照寿命；
  刷新失败则保留已发生的失败调用审计、中止本次决策且不下单。任一节点成功即结束重试、清零
...
- **正式运行启动试跑**：用户先通过独立的 `POST /api/engine/probe` 试跑，再通过
  `POST /api/engine/start` 启动。试跑使用当前排名
  ...
  推算周期耗时、总负载和最大安全标的数。后端从读取首份真实快照开始即通过
  `/api/status.startup_probe` 发布当前第几次、已完成次数及各 Provider 已完成样本的耗时；首页在
...
- `GET /api/live-runs/performance` 按开仓成交把仓位归属到正式运行，总盈亏为该运行仓位的已实现
  ...
  的已实现盈亏；部分平仓后剩余数量继续实时估值。胜率按“盈利平仓笔数 / 已完成平仓笔数”计算，
  手动平仓盈利计胜、亏损计负，零笔平仓显示为空。前端与账户数据一起每 5 秒刷新。
