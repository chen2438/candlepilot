...
| 总览 | 04 决策与风控 | 只展示归属于正式运行的决策，按持久化运行分组并显示运行编号、起止时间、状态、配置摘要、决策数与停止原因；每个运行组可独立折叠，运行中默认展开，已结束运行默认收起，展开后显示该次运行的全部决策；组内将 LLM 意图、硬风控与执行尝试合并为一条审计事件，可按下单成功、下单失败、否决、HOLD、仅推理筛选，并展开失败阶段、交易所错误、紧急回补和损失估算；不提供几乎不会持久存在的 `approved` 单独筛选按钮 |
