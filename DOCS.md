...
| 标签页 | 面板 | 内容 |
|---|---|---|
| 总览 | 引擎控制（hero）| 系统状态、分析周期、每周期标的数、正式决策硬超时、一次真实批量试跑的容量与调用详情、当前周期/标的/阶段，以及相互独立的试跑、启动、停止、紧急熔断按钮；宽屏为状态与控制两栏，四个控制按钮并排；下方实时显示本次或上次运行的 Token、等效成本、调用数与时长 |
