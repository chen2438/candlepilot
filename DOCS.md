...
- 每个周期在内部仍按标的串行，但整轮另受该周期自身的绝对时长预算约束；到下一周期边界仍未
  ...
  控制台运行区直接展示，不再出现后端卡住而界面只显示“运行中”的盲区。
...
- **逐条决策可查（`GET /api/backtests/{id}/decisions`）**：只报总数时，「0 笔交易」有四种
  ...
  调用和最多两次重试全部保留；控制台按浏览器本机时区显示到秒，并写明重试次数及每次重试的发出
...
- **控制台（回测发起）**：试跑成功后自动估算耗时；周期与模型为多选 chip；结果按
  ...
  **「与实盘的差异」对照表**（下单/订单流/价差/选币/特征/风控六项），默认收起。
  ...
  逐字折行后把整条运行记录异常撑高。
  试跑成功取得所有参与模型的 5 个样本后，控制台立即自动请求并显示耗时估算，不再要求用户点击
...
相关接口共享 1 秒查询缓存，将同一轮控制台并发轮询合并为一组账户摘要与持仓风险签名请求。
订单与成交接口始终读取本地执行审计记录。
