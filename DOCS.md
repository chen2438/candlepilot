...
- 特征：**5m/15m/30m** 共享 EMA、RSI、ATR、收益率、成交量，以及 20/50 根区间高低点与
  区间内位置（`range_position_50`，0 为区间底、1 为区间顶）；另有基差、持仓量、
  20 档盘口与近期成交失衡等微观结构特征。每个周期获取最近 200 根 K 线、排除未收盘 K 线
  后计算，连同日线结构位合计 53 个快照特征；LLM 接收特征值而不是原始 K 线数组。
...
- **喂给模型的周期 = 规则用到的周期**（`DECISION_FEATURE_INTERVALS`）。1m **不进** LLM
  payload：所有入场形态点名的都是 5m/15m/30m，没有一条规则读 1m，而没有分配角色的数据比
...
- 5m/15m/30m 对齐调度；每周期分析候选池前 N 个标的并额外包含全部已有持仓，确保掉出候选池的
  ...
  并发开仓，同向增仓须显式 `ADD`。
- **可选分析周期**：用户可自由选择分析 5m/15m/30m 的任意子集（默认全部）；1m 既不触发
...
（同时覆盖 5m/15m/30m 的全部决策时刻）采样实盘快照用的那几个端点并写下来。
...
- **喂给模型的 payload 与实盘同构**：同一套 `FeaturePipeline` 组装 5m/15m/30m 前缀特征
...
- **撮合**：入场从**下一根** K 线开始撮合（模型基于已收盘 K 线推理，在该 K 线内成交即读未来）。
  ...
  5m/15m/30m 三个决策也只结算一次；入场所在 5m K 线到下一时刻才结算，所以该 K 线内触发的
...
- **超时试跑（`POST /api/backtests/probe`）**：回测是数百次调用压在一个固定超时上，而合适的
  ...
  **每次调用一落地即可见**，并显示当前这次已等多久：探测等的正是它要测量的东西，
  ...
  `timeout_seconds` 随 run 落库：失败次数脱离它就无从解释。
