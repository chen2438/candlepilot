...
- **正式运行启动试跑**：每次 `POST /api/engine/start` 都必须在真正启动调度和交易前，用当前排名
  ...
  启动请求尚未返回时通过状态 WebSocket 实时显示 `0/3` 至 `3/3` 进度，完成后再切换为容量摘要。
...
- **正式运行边界持久化**：每次通过启动检查后都会创建唯一 `live_run_id`，本次 Provider 成功、
  ...
  `running` 记录会在下一次服务启动时明确标为 `interrupted`，不冒充仍在运行。
...
- **账户盈亏回报率与成交额**：持仓的未实现盈亏同时显示其相对于当前初始保证金的回报率
  （`unrealized_pnl / margin_used`）。成交明细以 `实际成交数量 × 实际成交均价` 显示 USDT
...
- SQLite 表：`live_runs`（正式运行边界、配置快照、起止账户权益快照与停止原因）、`inferences`（模型推理， nullable
...
- 正式运行开始和停止时分别保存 Binance 账户权益快照；`GET /api/live-runs/performance` 对运行中的
  会话使用最新账户权益、对已结束会话使用停止快照，返回两者相对开始权益的总盈亏，因此包含该
  时点全部已实现盈亏及未平仓仓位的未实现盈亏。接口同时把成交审计中的止损、止盈、模型减仓/
  平仓和手动平仓追溯至其正式运行，按“盈利平仓笔数 / 已完成平仓笔数”计算胜率；零笔平仓或功能
  启用前没有起止权益快照的历史运行返回空值，不猜测结果。前端与账户数据一起每 5 秒刷新。
...
| 标签页 | 面板 | 内容 |
|---|---|---|
| 总览 | 引擎控制（hero）| 系统状态、分析周期、每周期标的数、正式决策硬超时、三次真实试跑容量摘要、当前周期/标的/阶段、启动/停止/紧急熔断；宽屏为状态、指标、控制三栏，状态栏保留最小宽度，控制按钮并排以降低卡片高度；下方实时显示本次或上次运行的 Token、等效成本、调用数与时长 |
| 总览 | 01 模型接入 | 宽屏左栏。每张 Provider 卡片内集成接入状态；外部模型可配置模型与推理强度，本地规则显示固定版本与零 Token/成本；下方提供有序主备路由及当前承载/冷却状态 |
| 总览 | 02 硬风控边界 | 宽屏右栏顶部，只读展示 1% 单笔风险、4% 组合止损风险、原始盈亏比必须大于 1.3:1、保证金与持仓模式等不可修改参数 |
| 总览 | 03 动态候选池 | 宽屏右栏紧接硬风控，填充模型列表右侧空间；展示全市场扫描结果，可手动刷新；默认只显示评分前 5 个，可展开查看全部 |
| 总览 | 04 决策与风控 | 只展示归属于正式运行的决策，按持久化运行分组并显示运行编号、起止时间、状态、配置摘要、决策数、停止原因、包含结束时未实现盈亏的总盈亏及已完成平仓胜率；运行指标每 5 秒刷新。每个运行组可独立折叠，运行中默认展开，已结束运行默认收起，展开后显示该次运行的全部决策；每条决策直接显示从模型调用开始到风控/执行审计完成的总耗时。组内将 LLM 意图、硬风控与执行尝试合并为一条审计事件，可按下单成功、下单失败、否决、HOLD、仅推理筛选，并展开失败阶段、交易所错误、紧急回补和损失估算；不提供几乎不会持久存在的 `approved` 单独筛选按钮 |
...
账户启动对账失败均不会创建运行会话或启动调度。并行试跑中任一 Provider 失败时，后端会取消并
等待同组其他调用完整退出后再恢复超时并返回失败，不能遗留继续计费或占用单并发锁的后台调用。
