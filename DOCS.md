...
- **确定性移动止损**：移动止损不交给 LLM。独立看护每 5 秒读取 Demo 测试网的实时标记价、
  加权开仓价与真实交易所止损，以首次观察到的入场风险距离为 `1R`；价格达到 `+2R` 后，止损按
  历史最有利标记价后方 `1R` 跟踪。多仓止损只上移、空仓只下移，候选按 Demo `tickSize` 向保守
  方向对齐，并且每次至少改善 `max(1 tick, 0.1R)` 才更新，避免行情噪声导致高频撤挂。固定止盈
  始终保留。`shadow` 模式只把候选写入 `trailing_stop_events`，`live` 才替换交易所止损；默认
...
| `CANDLEPILOT_TRAILING_STOP_MODE` | 确定性移动止损模式：`off` / `shadow` / `live`，默认 `shadow`；固定为 +2R 激活、距最有利标记价 1R，只有 `live` 修改交易所止损 |
...
`GET /api/status` 的 `scheduler.trailing_stop` 返回当前模式、固定 R 参数、管理/已激活仓位数和最近事件。
