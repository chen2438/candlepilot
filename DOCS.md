...
> 最后更新：2026-07-16（支持同时接入多个 Custom API）
...
- **多个 Custom API**：除上面这个由扁平 `CANDLEPILOT_CUSTOM_LLM_*` 定义的端点（名为
  `openai-compatible`，保持向后兼容）外，可用 `CANDLEPILOT_CUSTOM_LLM_PROVIDERS_JSON` 再定义
  最多 8 个端点。每项需唯一小写 `id` 与 `base_url`，可选 `api_key` / `model` /
  `reasoning_effort` / `wire_api` / `require_api_key` / `extra_headers`，各端点**互相独立**
  （各自的地址、密钥、模型与协议）。注册名为 `openai-compatible:<id>`，在主备路由中可写
  `custom:<id>`（大小写不敏感）。未知键、非法/重复 `id`、非法 wire_api 或受保护请求头都会在
  启动时报错，避免拼写错误被静默忽略。
...
| `CANDLEPILOT_CUSTOM_LLM_PROVIDERS_JSON` | 额外自定义端点 JSON 数组（最多 8 个），每项需唯一 `id` 与 `base_url`，注册为 `openai-compatible:<id>` |
...
`POST /api/cadences`、
`POST /api/candidates-per-cycle`、`POST /api/run-limits`、`POST /api/engine/start`、
