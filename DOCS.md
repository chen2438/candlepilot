...
`/api/status` 的 `auto_stop_reason`，控制台顶部横幅展示。独立轮询是必要的：否则 4h 周期会把
...
- 硬风控（不可由模型修改）：逐仓、单向净仓、最高 10x、单笔计划亏损 ≤ 权益 2%、
  最多 8 个仓位、总保证金占用 ≤ 60%；日内净亏损 8% 触发熔断。测试网组合从 Binance 收益
  流水汇总 UTC 当日 `REALIZED_PNL`、`COMMISSION`、`FUNDING_FEE`，再加账户当前未实现盈亏；
  充值和划转不算作交易盈亏。控制台账户看板显示同一个数值，避免界面与硬风控口径分离。
...
- **未成交入场单也属于组合暴露**：每次交易前从 Binance 普通与 Algo 挂单接口回读非
  ...
  计入 60% 组合上限。该检查不只发生在启动对账，运行中的每次预交易刷新都会重新执行。
...
- **决策记录可翻页、可筛选**：控制台默认显示最新 50 条并保持实时更新，「加载更早」按 `before_id`
...
- **独立于引擎**，控制台手动启停（`POST /api/collector/start|stop`，`GET /api/collector`）。
...
- **多模型对比**：同一窗口跑 1–4 个模型，跨模型并行（同模型内串行），逐模型独立报告
  收益/胜率/回撤/盈亏比/交易数。一个模型失败不影响其他模型的结果。
- **成本护栏**：`POST /api/backtests/estimate` 先算调用数与预计耗时（**用本机审计日志里的
  真实延迟**，不是拍脑袋）。上限按**耗时 8 小时**而非调用数——同样 2592 次调用，快端点 20 分钟、
  慢端点 17 小时，按次数限制对前者过松、对后者过紧。窗口 ≤ 3 天、≤ 5 标的、≤ 4 模型。
- **异步执行**：真实窗口是数小时，不可能同步返回。`POST /api/backtests` 返回 **202 + run_id**，
  ...
  每个已完成模型调用还会累计并持久化输入、缓存输入、缓存写入、输出与总 Token；控制台在模型行
  ...
  时 Provider 没有返回可审计的精确耗时，因此失败调用只计失败数，不伪造进平均值。
- **超时试跑（`POST /api/backtests/probe`）**：回测是数百次调用压在一个固定超时上，而合适的
  超时值因端点而异——猜一个 45s 曾让一次运行 12 次决策丢掉 5 次，却仍报出一个整洁的 0% 收益。
  发起前对**每个模型跑 3 次真实决策**（真实端点、真实 payload、真实计费），量出它实际要多久，
  由用户决定本次运行的超时。试跑期间超时放宽到 `PROBE_CEILING_SECONDS`（180s）：
  **用当前超时去试只会复现超时**，一个真需要 70s 的端点只会回答"45s 超时"，什么也学不到。
  建议值 = 最慢成功 × 1.5；3 个样本描述不了长尾，因此这是**起点而非安全边界**。
  全部失败则不给建议——答案不是更大的数字，而是这个端点跑不动这次回测。
  **每次调用一落地即可见**，并显示当前这次已等多久：探测等的正是它要测量的东西，
  一个只在结束时才吐结果的探测，在上限下可以沉默九分钟，和卡死无法区分。
...
- **逐条决策可查（`GET /api/backtests/{id}/decisions`）**：只报总数时，「0 笔交易」有四种
  ...
  控制台点模型名即展开，配色沿用实盘决策面板（同样的三个含义没必要换一套颜色）。每次 LLM
  调用完整返回后，该条决策立即落库；展开中的运行每 3 秒自动重读决策和结果，所以可实时看到
  已完成的整条记录，但不会展示模型尚未返回的半成品。决策随 run 级联删除。
- **不可信而非「已完成」（`MAX_FAILURE_RATE = 10%`）**：失败的调用不是 HOLD，是一次**没发生过的
  决策**——模型没看过那根 K 线，于是这次运行评估的是一个随机缺席了部分窗口的策略。
  超过阈值的运行标记为 `unreliable` 并写明缺口，而不是与干净运行并列为 `completed`。
  结果仍然展示（是标注，不是隐藏）；任一模型超标即整轮标记，因为并排比较正是回测的意义所在。
- **控制台**：独立「回测」页，页首即采集器面板（启停 + 已录概览）——放在一起是因为
...
- **控制台（回测发起）**：发起前可先估算耗时；周期与模型为多选 chip；结果按
  ...
  **「与实盘的差异」对照表**（下单/订单流/价差/选币/特征/风控六项），默认收起。
  ...
  页面布局按“数据准备 → 回测参数 → 真实回测/试跑 → 执行 → 运行记录”分层；采集、试跑、估算与
...
| 账户 | 06 账户与订单 | 展示币安测试网账户的权益与持仓，以及本地审计的订单成交；每 5 秒刷新 |
