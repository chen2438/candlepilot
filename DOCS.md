...
- **逐条决策可查（`GET /api/backtests/{id}/decisions`）**：只报总数时，「0 笔交易」有四种
  ...
  控制台点模型名即展开，配色沿用实盘决策面板（同样的三个含义没必要换一套颜色）。
  按 `DECISION_BATCH` 批量写入——runner 每次决策本就要写一次进度，搭同一趟车；
  收尾必须 drain 最后一批，否则每次运行的尾巴全部丢失。决策随 run 级联删除。
