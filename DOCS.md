...
- 特征：**5m/15m/30m** 共享 EMA、RSI、ATR、收益率、成交量，以及 20/50 根区间高低点与
  ...
  后计算，连同日线结构位合计 50 个快照特征；LLM 接收特征值而不是原始 K 线数组。
...
- 区间高低点是模型判断"价格是否已延伸""是否收复/拒绝参考位"的**唯一依据**——均线只说方向、
  ...
  该形态即视为不成立。入场形态已改写为同时读日线位：形态 (1) 禁止在
  `1d_range_position_20` 接近/超过 1 时做多、接近/低于 0 时做空；形态 (2) 视日线位守住为
  比日内位更强的证据；形态 (3) 只有在日线位上的拒绝才值得满仓入场。
