...
- `GET /api/live-runs/performance` 按开仓成交把仓位归属到正式运行，总盈亏为该运行仓位的已实现
  ...
  的已实现盈亏；部分平仓后剩余数量继续实时估值。前端将该指标明确标为“已平仓胜率”，按
...
| 总览 | 04 决策与风控 | 只展示归属于正式运行的决策，按持久化运行分组并显示运行编号、起止时间、状态、配置摘要、决策数、停止原因、该运行仓位的已实现加当前未实现总盈亏及已完成平仓胜率；停止后手动平仓仍更新原运行的盈亏和胜负，运行指标每 5 秒刷新。每个运行组可独立折叠，运行中默认展开，已结束运行默认收起，展开后显示该次运行的全部决策；每条决策明确显示同批标的共享、不可逐条相加的批次耗时，后续风控或执行额外增加至少 0.10 秒时另显示整笔耗时。组内将 LLM 意图、硬风控与执行尝试合并为一条审计事件，可按下单成功、下单失败、否决、HOLD、仅推理筛选，并展开失败阶段、交易所错误、紧急回补和损失估算；不提供几乎不会持久存在的 `approved` 单独筛选按钮 |
