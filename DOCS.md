...
- **正式运行启动试跑**：用户先通过独立的 `POST /api/engine/probe` 试跑，再通过
  ...
  实时行情快照、当前测试网组合及当前完整 Provider 路由，对每个 Provider 连续执行 3 次真实的
  **全标的批量意图**。试跑不经过风控、不下单、不写入正式决策审计，但外部
  调用会产生真实 Token/计费。任一调用失败或超时即拒绝启动。系统取所有 Provider、所有三次
  样本中最慢的“全部行情 + 一份账户 + 一次批量意图生成”真实墙钟耗时；超过任一所选周期即返回
  ...
  `/api/status.startup_probe` 发布当前第几次、已完成次数、批量标的列表/数量及各 Provider 已完成
  样本的耗时；首页进度显示“标的数量 · 周期”，悬浮可查看完整标的列表，不再用批次首个标的冒充
  当前单标的；首页在
  试跑请求尚未返回时通过状态 WebSocket 实时显示 `0/3` 至 `3/3` 进度，完成后再切换为容量摘要。
...
- 硬风控（不可由模型修改）：逐仓、单向净仓、最高 10x、单笔及单标的计划亏损 ≤ 权益 1%、
  ...
  初始保证金 ≤ 权益 10%；不设置持仓标的数量上限；日内净亏损 5% 触发熔断。组合止损风险逐仓
...
- Provider Prompt 不提及盈亏比目标或硬阈值，只要求开仓与 `ADD` 同时返回止损和止盈；盈亏比由
  ...
  买卖价和保守滑点仍参与止损风险定量，避免低估仓位亏损。`REDUCE/CLOSE` 不受盈亏比、日亏熔断或
...
- 每次新推理都审计结构化行情/组合输入、实际 Prompt、Provider 原始输出和 token usage；批量
  ...
  复制，避免长文本一次撑高整个决策。所有详情只经 localhost API 提供且不包含 Provider 或币安凭据。
...
| 标签页 | 面板 | 内容 |
|---|---|---|
| 总览 | 引擎控制（hero）| 系统状态、分析周期、每周期标的数、正式决策硬超时、三次真实试跑容量摘要、当前周期/标的/阶段，以及相互独立的试跑、启动、停止、紧急熔断按钮；宽屏为状态与控制两栏，四个控制按钮并排；下方实时显示本次或上次运行的 Token、等效成本、调用数与时长 |
| 总览 | 01 模型接入 | 宽屏左栏。每张 Provider 卡片内集成接入状态；外部模型可配置模型与推理强度，本地规则显示固定版本与零 Token/成本；下方提供有序主备路由及当前承载/冷却状态 |
| 总览 | 02 硬风控边界 | 宽屏右栏顶部，只读集中展示当前候选标的数、10 倍最大杠杆、5% 日亏熔断、1% 单笔风险、4% 组合止损风险、原始盈亏比必须大于 1.3:1、保证金与持仓模式等边界 |
| 总览 | 03 动态候选池 | 宽屏右栏紧接硬风控，填充模型列表右侧空间；展示全市场扫描结果，可手动刷新；默认只显示评分前 5 个，可展开查看全部 |
| 总览 | 04 决策与风控 | 只展示归属于正式运行的决策，按持久化运行分组并显示运行编号、起止时间、状态、配置摘要、决策数、停止原因、该运行仓位的已实现加当前未实现总盈亏及已完成平仓胜率；停止后手动平仓仍更新原运行的盈亏和胜负，运行指标每 5 秒刷新。每个运行组可独立折叠，运行中默认展开，已结束运行默认收起，展开后显示该次运行的全部决策；每条决策直接显示从模型调用开始到风控/执行审计完成的总耗时。组内将 LLM 意图、硬风控与执行尝试合并为一条审计事件，可按下单成功、下单失败、否决、HOLD、仅推理筛选，并展开失败阶段、交易所错误、紧急回补和损失估算；不提供几乎不会持久存在的 `approved` 单独筛选按钮 |
...
接口同步完成上述 3 次真实试跑和容量校验，但不创建运行会话或启动调度。并行试跑中任一 Provider
