...
- 代码只保留当前实际使用的边界：`DecisionProvider` 抽象负责模型接入，`FeaturePipeline`、
  `TradingEngine` 与 `AggressiveRiskPolicy` 分别负责特征、编排和确定性风控；行情与 Broker 当前明确
  为 Binance 实现，不为尚未支持的交易所预建空适配器层。
...
- **突破确认是独立硬规则**：结构总门槛继续保持 `shadow` 时，`TREND_BREAKOUT` 或 `BREAKOUT`
  ...
  升级成全量 `enforce`。
- **止损后同标的冷却**：CandlePilot 自己的止损单成交后，该 symbol 在 90 分钟内禁止新的
  `OPEN_LONG`、`OPEN_SHORT` 或 `ADD`。截止时间由持久化 `ORDER_TRADE_UPDATE` 中客户端单号以
  `cp-` 开头、`-sl` 结尾的真实 `FILLED/TRADE` 事件重建，因此进程或 VPS 服务重启不会清空冷却；
  当前映射放入 `PortfolioState.stop_loss_cooldown_until`，LLM 在输入中可见且硬风控会再次否决，
  正式运行快照也会原样保存供回放；历史模拟交易触发止损时，同样在模拟账户内产生并按时间推进
  清除这段冷却。手工平仓与止盈成交不触发这条冷却。
...
- **确定性移动止损**：移动止损不交给 LLM。独立看护每 5 秒读取 Demo 测试网的实时标记价、
  ...
  每组独立保存激活状态与最近候选，互不覆盖，也不会修改交易所订单。`live` 为避免候选争抢同一保护单，
...
把完整 `PortfolioState` 固化为该次运行的起始账户；每轮 Provider 路由最终采用哪次输入，就在
...
同一批输入共享 UUID `batch_id`，保留正式运行的批量决策边界，后续升级回放调用方式时不必靠相近
时间戳猜哪些标的原本属于同一次 Provider 调用。

这套记录与手动盘口采集互补：手动采集器适合在不运行策略时积累任意连续窗口；正式决策数据集只
记录当次运行实际分析过的动态候选与已有持仓，但同时保存完整特征、账户和合约规则，精度更高且
无需用户操作。它独立于推理审计表，避免 Provider 重试条数、审计详情清理或展示结构决定回放数据
的语义。删除“模型调用与决策”会删除对应 `live_run`，数据库外键再级联删除其正式决策数据集。
...
顶部标签栏将功能分为六页，避免单页过长；WebSocket 状态/决策推送与账户轮询在所有标签页
...
| 总览 | 02 硬风控边界 | 宽屏右栏顶部，整块默认展开并可独立折叠，折叠后保留编号、标题和只读摘要；切换标签页后保留本次页面会话中的折叠状态；集中展示当前候选标的数、10 倍最大杠杆、当前运行实例的滚动 24 小时亏损熔断比例、1% 单笔风险、4% 组合止损风险、原始盈亏比必须大于 1.3:1、保证金与持仓模式等边界 |
...
| 账户 | 06 账户与订单 | 集中展示币安测试网配置/用户流/启动对账状态、钱包与权益、交易所实时持仓；持仓按标记价显示 USDT 名义价值与初始保证金，并按开仓均价和多空方向显示止盈/止损百分比及原始盈亏比（不含手续费和滑点）；表格将方向/杠杆、持仓价值/保证金、均价/标记价分别合列，并将止损、止盈分列紧凑展示；同时展示含开仓、止损、止盈、回补和手动平仓的统一成交生命周期，以及五组 shadow 参数卡片和最近 100 条候选/应用/失败记录；每 5 秒刷新；引擎停止时可二次确认后按标的市价平仓 |
...
- **重启后端**（`POST /api/restart`，前端设置页按钮）：用当前 `.env` 重新执行后端进程，让保存
  的设置生效。**引擎、调度器、Provider 探测、回测或盘口采集器仍在运行时都会被拒绝（409）**，
  避免打断交易或留下不完整任务；接受重启后立即拒绝新的写请求，并按调度器、引擎、采集器、行情、
...
- **网页一键更新**（`GET /api/update/status`、`POST /api/update`，前端设置页「软件更新」）：仅在
  ...
  `sudo -n /usr/local/sbin/candlepilot-web-update` 请求独立的 `candlepilot-update.service`；更新任务
  不在应用服务的 cgroup 内，因此停止 `candlepilot.service` 不会杀掉更新器。引擎、调度器、
  ...
  应用用户的 sudo 权限只允许启动这个 root 所有、拒绝任何参数的 launcher；worker 和安装器副本
  同样由 root 持有，绝不以 root 运行应用用户可写目录里的脚本。更新仍只接受配置的 GitHub origin
  与分支快进，并沿用下文的备份、健康检查和失败回滚。
- `.env` 路径默认为工作目录下的 `.env`，可用 `CANDLEPILOT_ENV_FILE` 指定。

### 4.12 数据管理

- 前端「数据管理」面板与 `POST /api/history/clear` 可按类别删除历史数据：
  模型调用与决策（含对应正式运行的自动回放数据）、风控决策、订单成交、测试网事件、告警历史、移动止损记录、**回测记录**、**盘口采集**、
...
安装/刷新网页更新专用的 root-owned launcher、worker、安装器副本、配置、sudoers 精确命令授权和
`candlepilot-update.service`。这些文件从当前 Git 提交读取，worker 只执行
...
服务、root 更新助手、精确 sudoers 授权、更新状态/日志、Nginx 站点、TLS 配置和应用目录
