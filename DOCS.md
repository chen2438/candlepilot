...
- 区间高低点是模型判断"是否收复/拒绝参考位"的依据——均线只说方向、不说位置。Prompt 逐条点名
  ...
  `range_high_20` 包含最新 K 线，只适合描述当前区间；真正的突破判断必须读取排除最新 K 线的
  `prior_range_*` 与对应 `breakout_above_20` / `breakdown_below_20`。`last_swing_*` 只有在对应
  `*_confirmed=1` 时才是已确认摆动点，避免让模型把仍会重绘的末端极值冒充结构。依赖未来突破或
  收复的市价单必须 `HOLD`；只有明确限价触发点才能进入本地待触发流程。
...
- **结构入场门槛**：默认 `shadow`，对每个经过其他实时硬风控的开仓/加仓意图机械检查并随
  ...
  市价单触发价已经越过，突破单还必须与 `breakout_above_20` / `breakdown_below_20` 和旧边界吻合；
