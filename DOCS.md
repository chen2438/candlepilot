...
| 账户 | 06 账户与订单 | 集中展示币安测试网配置/用户流/启动对账状态、钱包与权益、交易所实时持仓；持仓按标记价显示 USDT 名义价值与初始保证金，并按开仓均价和多空方向显示止盈/止损百分比及盈亏比；表格将方向/杠杆、持仓价值/保证金、均价/标记价分别合列，并将止损、止盈分列紧凑展示；同时展示含开仓、止损、止盈、回补和手动平仓的统一成交生命周期；每 5 秒刷新；引擎停止时可二次确认后按标的市价平仓 |
