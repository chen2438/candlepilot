...
- 趋势突破只有在连续两根已收盘 K 线守住同一突破前边界后才可入场。
- 止损成交后，同币种 90 分钟内不得重新开仓或加仓；冷却由成交审计重建，可跨服务重启恢复，
  历史回测也按相同规则推进。
- 每次运行显式选择唯一 Provider，统一取消、超时、同 Provider 重试和单并发限制；币安密钥永不进入 LLM 子进程。
...
- 正式引擎自动保存实际批量决策的完整行情、账户和合约规则，可按原批次精确回放；普通历史回测
  只使用交易所历史 K 线，并明确告知模型订单流缺失，不再提供独立盘口采集器。
...
- 模型运维面板展示 Provider 延迟、调用量与错误率；测试网账户状态与持仓集中在账户页。
- 存活/就绪健康检查、JSON 结构化日志、运行指标，以及告警规则与本地去重通知/历史。
- VPS 日志使用独立 systemd journal 命名空间，可在设置页二次确认后只清理 CandlePilot 日志，
  不影响 SSH、Nginx 或其他服务。
...
`candlepilot` 启动时会自动读取当前目录的 `.env`（`doctor`、`serve` 都适用），
无需手动 `source`。已在 shell 中 `export` 的变量优先级更高，会覆盖 `.env`。

浏览器访问 [http://127.0.0.1:8000](http://127.0.0.1:8000)。在前端互斥选择本次唯一 Provider，
选择唯一的决策周期（5m/15m/30m/1h/4h，默认 15m）和每周期
分析的候选标的数（默认前 5，范围 1–20），刷新候选池。持续启动前先点击「试跑」；正式运行试跑会对
所选 Provider 使用当前真实行情和测试网账户执行 **1 次全标的批量调用**，不经过风控、不下单，
但外部调用会产生真实 Token/计费；页面展示模型、推理强度、批次耗时、动作分布、Token、等效成本
和逐标的意图。试跑成功只解锁一次「启动」，不会自动运行；修改周期、标的数、运行上限、Provider
选择或模型配置后必须重新试跑，成功启动也会消费本次试跑，停止后再次启动同样需要重跑。
...
总览 / AI 分析 / 账户 / 回测 / 运维 / 数据 / 设置七个标签页组织，实时状态推送在切换标签时保持不断。
...
为兼容现有部署，变量名仍是 `CANDLEPILOT_PROVIDER_CHAIN`，但值必须恰好选择一个 Provider，不能
再用逗号配置主备。只使用面向配置的短名称 `local`、`local-structure`、`local-flow`、
`local-structure-flow`、`codex`、`claude-code` 与 `custom:<id>`；后端解析后才将它们规范化为状态/API
中看到的 `local-rule`、`local-structure-shadow`、`local-flow-shadow`、`local-structure-flow-shadow`、
`codex-auth`、`claude-code-auth` 与 `openai-compatible:<id>`。不要把内部注册名写回
...
安装器会同时部署受限的网页更新助手。以后仓库有更新时，先在网页中停止正式引擎，并等待独立 AI
分析、自动研究轮次、回测、Provider 试跑或 Codex 登录任务结束，再到「设置 → 软件更新」确认即可，
无需登录 VPS 终端。更新保留 `.env`、数据库、行情数据、
TLS 和模型登录状态，只接受 `main` 的快进更新，并在依赖安装、前端构建或健康检查失败时恢复旧版本；
更新前会在 `/var/backups/candlepilot` 保存配置、`.env` 当前指向的 SQLite 数据库和原提交号。

从旧版本首次启用网页更新时，仍需最后执行一次上面的 `curl | sudo bash` 安装命令来安装 root 助手；
即使代码已经是最新版本也会补装。此后更新都可在网页完成。固定、无参数的 launcher 只写入
systemd 监听的更新请求文件，不调用 `sudo`；主服务继续保留 `NoNewPrivileges`。实际 worker 和
安装器副本均由 root 持有，不会以 root 执行应用用户可写目录里的脚本。

「设置 → 服务器备份」可查看安装器备份的时间、占用和来源提交，并在二次确认后删除旧备份；最新
