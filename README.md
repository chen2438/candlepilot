...
- 决策与风控面板：将 LLM 意图和对应硬风控结果合并展示、筛选并展开审计详情；账户页专注于
  权益、持仓和订单成交。
...
OpenAI-compatible Chat Completions 或 Responses 服务：
...
CANDLEPILOT_CUSTOM_LLM_WIRE_API=responses
CANDLEPILOT_DEFAULT_PROVIDER=openai-compatible
```

外部 Base URL 必须使用 HTTPS；本机回环服务可用 HTTP。Key 只由后端从环境变量读取，
不会通过控制台/API 回传或写入日志、数据库。默认协议是 `chat-completions`；使用 Responses
兼容服务时设为 `responses`。不使用 Bearer Key 的服务可将
`CANDLEPILOT_CUSTOM_LLM_REQUIRE_API_KEY=false`，并通过
`CANDLEPILOT_CUSTOM_LLM_EXTRA_HEADERS_JSON` 配置服务商要求的自定义请求头。配置后可在控制台
点击「测试」验证实际调用。
