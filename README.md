...
- Codex App Auth：优先检测 ChatGPT App 的内置 `codex`，
...
- 回测前可先「试跑 3 次决策」量出每个端点的真实延迟，据此决定本次超时——固定超时猜错会
  让决策成批丢失。丢失超过 10% 的运行标记为**不可信**而非「已完成」：失败的调用不是 HOLD，
  是一次没发生过的决策。
...
主备顺序，按需选择要分析的决策周期（5m/15m/30m/1h/4h 的任意子集，默认全部）和每周期
分析的候选标的数（默认前 5，范围 1–20），刷新候选池，然后启动引擎。启动后调度器
只对齐并运行被选中的 K 线周期，且每周期只评估排名靠前的 N 个标的。前端按
