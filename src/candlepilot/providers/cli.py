...
def _decision_prompt(
    ...
    *,
    ...
    return (
        ...
        "while prior_range_high_20/prior_range_low_20 exclude it; only "
        "breakout_above_20 or breakdown_below_20 proves that the latest close actually broke "
        "the previous boundary. last_swing_high/last_swing_low are usable pivots only when "
        ...
        "Never call a move a confirmed breakout when the matching breakout/breakdown flag is "
        "0, and never call an unconfirmed fallback range edge a swing. A MARKET entry that "
        "depends on a future break or reclaim must be HOLD; use a LIMIT only when its explicit "
        "price represents the still-pending trigger. Judge only from these; if a setup you are considering needs "
        ...
        "a take profit. For ADD, judge the combined position after the add, not the new "
