...
def _decision_prompt(
    ...
    *,
    ...
    return (
        ...
        "least one of 15m/30m align, price is not already extended, and volume/order flow "
        "does not materially contradict; (2) pullback: the higher-timeframe trend remains "
        "intact and the retracement stabilizes or reclaims a nearby reference with renewed "
        "participation; (3) reversal: a structure reclaim/rejection plus momentum or flow "
        "confirmation. Overbought or oversold readings alone are never reversal confirmation. "
        "For an existing position, ADD requires the same entry confluence; REDUCE or CLOSE "
        "when its invalidation is reached or opposing evidence is confirmed. Otherwise HOLD. "
