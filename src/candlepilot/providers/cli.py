...
def _decision_prompt(
    ...
    *,
    ...
    return (
        ...
        "Intraday features are per interval, prefixed 5m/15m/30m. Trend is ema_spread and "
        ...
        "not at an intraday extreme -- so they outrank the intraday levels wherever the two "
        "disagree. They are levels, not a trend: the daily bar says nothing here about "
        "direction, and must not be read as a directional bias. "
        ...
        "least one of 15m/30m align, volume/order flow does not materially contradict, and "
        "price is not already extended -- neither into the intraday range edge nor into the "
        "daily one, so do not buy with 1d_range_position_20 near or above 1, and do not sell "
        "it near or below 0; (2) pullback: the higher-timeframe trend remains intact and "
        "price stabilizes at or reclaims a nearby level with renewed participation, counting "
        "a daily level held as stronger evidence than an intraday one; (3) reversal: price "
        "rejects or reclaims a level plus momentum or flow confirmation, and a rejection at "
        "a daily level is the only reversal worth a full-size entry. Overbought or oversold "
        "readings alone are never reversal confirmation. "
