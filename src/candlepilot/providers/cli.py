...
def _decision_prompt(
    ...
    *,
    ...
    return (
        ...
        "portfolio.position_entry_context 提供两类可审计记忆：所有当前交易所持仓仍未平掉的开仓/"
        "加仓理由，以及本次正式运行内全部已成交开仓/加仓的理由和后续退出。opened_in_current_run "
        "表示是否属于本次运行，currently_open 和 status 表示当前状态；exits 中 kind=take_profit "
        "或 stop_loss 分别代表止盈或止损，manual_close/model_close/model_reduce/rescue_close/"
        "other_close 必须按原值理解，不得冒充止盈止损。realized_pnl 是该退出分配给对应开仓批次的"
        "已实现盈亏。rationale=null 或 status=UNKNOWN 表示审计不足，不得猜测缺失理由或退出结果。"
